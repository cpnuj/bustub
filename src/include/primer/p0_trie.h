...
class TrieNode {
 public:
  ...
  TrieNode &operator=(TrieNode &&other_trie_node) noexcept {
...
class Trie {
 private:
  ...
  template <typename T>
  T GetInternal(const std::string &key, size_t idx, std::unique_ptr<TrieNode> *curr, bool *succ) {
    if (idx == key.size()) {
      if (!(*curr)->IsEndNode()) {
        return {};
      }

      auto node = dynamic_cast<TrieNodeWithValue<T> *>(curr->get());
      if (node != nullptr) {
        *succ = true;
    ...
    if (!(*curr)->HasChild(key[idx])) {
      return {};
    }

    return GetInternal<T>(key, idx + 1, (*curr)->GetChildNode(key[idx]), succ);
  }

  template <typename T>
  bool InsertInternal(const std::string &key, size_t idx, std::unique_ptr<TrieNode> *curr, T value) {
    if (idx == key.size()) {
      if (!(*curr)->IsEndNode()) {
    ...
    if (!(*curr)->HasChild(key[idx])) {
      (*curr)->InsertChildNode(key[idx], std::make_unique<TrieNode>(key[idx]));
    }

    return InsertInternal(key, idx + 1, (*curr)->GetChildNode(key[idx]), value);
  }

  bool RemoveInternal(const std::string &key, size_t idx, std::unique_ptr<TrieNode> *curr) {
    if (idx == key.size()) {
      if ((*curr)->IsEndNode()) {
        *curr = std::make_unique<TrieNode>(std::move(**curr));
        return true;
      }
      return false;
    }

    auto next = (*curr)->GetChildNode(key[idx]);

    if (next == nullptr) {
      return false;
    }

    bool succ = RemoveInternal(key, idx + 1, next);

    if (!(*next)->IsEndNode() && !(*next)->HasChildren()) {
      (*curr)->RemoveChildNode(key[idx]);
      return true;
    }
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    latch_.WLock();
    bool succ = key.empty() ? false : InsertInternal(key, 0, &root_, value);
    latch_.WUnlock();
    return succ;
  }
  ...
  bool Remove(const std::string &key) {
    latch_.WLock();
    bool succ = key.empty() ? false : RemoveInternal(key, 0, &root_);
    latch_.WUnlock();
    return succ;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    latch_.RLock();
    auto ret = GetInternal<T>(key, 0, &root_, success);
    latch_.RUnlock();
    return ret;
  }
};
